...
class SupabaseClient {
  ...
  async select(table, params = '') {
    return this.query(table, 'GET', null, `?${params}&order=ticket.asc`);
  }

  async insert(table, data) {
    return this.query(table, 'POST', data, '?select=*');
  }
  ...
  // Realtime subscription simulation (polling)
  subscribe(table, callback, interval = 2000) {
    const poll = async () => {
      try {
...
export default function QueueSystem() {
  ...
  useEffect(() => {
    ...
    subscriptionRef.current = supabase.subscribe('queue_entries', (data) => {
      setQueueData(data);
      setIsConnected(true);
    });

    // Subscribe to system state
    const stateSubscription = supabase.subscribe('system_state', (data) => {
      if (data.length > 0) {
  ...
  const loadQueueData = async () => {
    try {
      const data = await supabase.select('queue_entries');
      setQueueData(data);
      setIsConnected(true);
      setIsLoading(false);
    } catch (error) {
  ...
  const loadSystemState = async () => {
    try {
      const data = await supabase.select('system_state');
      if (data.length > 0) {
        setCurrentQueue(data[0].current_queue);
      }
    } catch (error) {
  ...
  const updateSystemState = async (newQueue) => {
    try {
      await supabase.update('system_state', 1, { current_queue: newQueue });
    } catch (error) {
  ...
  const joinQueue = async () => {
    ...
    try {
      if (isSupabaseConfigured) {
        await supabase.insert('queue_entries', newEntry);
        await loadQueueData();
      } else {
  ...
  const nextQueue = async () => {
    ...
    try {
      if (isSupabaseConfigured) {
        ...
        if (currentEntry) {
          await supabase.update('queue_entries', currentEntry.id, { status: 'completed' });
        }

        // Update next entry to current
        const nextEntry = queueData.find(item => item.ticket === nextNumber);
        if (nextEntry) {
          await supabase.update('queue_entries', nextEntry.id, { status: 'current' });
        }
  ...
  const resetQueue = async () => {
    ...
    try {
      if (isSupabaseConfigured) {
        await supabase.delete('queue_entries', 'ticket=gte.0');
